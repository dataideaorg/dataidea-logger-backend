...
class ApiKeyViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        # Return only active API keys for current user
        return ApiKey.objects.filter(user=self.request.user, is_active=True)

    def perform_create(self, serializer):
        # Associate new API key with current user
...
# ViewSet for event log messages
...
def create_event_log(request):
    ...
    """
    # Validate and save the log entry
...
def get_user_stats(request):
    ...
    # Get event logs distribution by level
...
# View for getting analytics data
@api_view(['GET'])
@permission_classes([permissions.IsAuthenticated])
def get_analytics_data(request):
    ...
    # Get logs filtered by user and optionally by project
    ...
    # Calculate LLM sources distribution
    ...
    if not project_id:
        ...
        # Combine counts from both log types
...
# View for downloading analytics data as CSV
@api_view(['GET'])
@permission_classes([permissions.IsAuthenticated])
def download_analytics_csv(request, data_type):
    ...
    # Write monthly data if requested
    ...
    # Write sources data if requested
    ...
    # Write levels data if requested
    ...
    # Write projects data if requested
...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        # Associate new project with current user
        serializer.save(user=self.request.user)
    ...
    def list(self, request, *args, **kwargs):
        ...
        # Add log counts for each project
